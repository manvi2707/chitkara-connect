...
const MessageBubble = ({ msg, isOwn }) => (
  <div className={`flex items-end gap-1.5 mb-2 ${isOwn ? "flex-row-reverse" : "flex-row"}`}>
    ...
    >
      <div
        className={`px-3 py-2 rounded-2xl text-sm leading-relaxed shadow-sm break-words w-full ${
          isOwn
    ...
    <div className="flex overflow-hidden bg-gray-50 h-full">
      ...
      `}>
        {activeConvo ? (
          <>
            {/* Thread header */}
            <div className="flex items-center gap-2 px-3 py-2.5 bg-white border-b border-gray-200 shadow-sm flex-shrink-0">
              <button
                onClick={() => {
                ...
                className="sm:hidden text-blue-600 p-1 -ml-1 text-xl leading-none"
              >←</button>
            ...
            <div className="bg-white border-t border-gray-200 px-3 py-2 flex-shrink-0">
              <form onSubmit={handleSend} className="flex items-end gap-2">
                <textarea
                  ...
                  rows={1}
                  className="flex-1 min-w-0 resize-none border border-gray-200 rounded-2xl px-3 py-2.5 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition max-h-28 overflow-y-auto"
                  style={{ minHeight: "40px" }}
                />
                <button
                  type="submit"
                  disabled={!messageText.trim() || sending}
                  className="bg-blue-600 hover:bg-blue-700 text-white w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 transition disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {sending ? (
              ...
              <p className="text-[10px] text-gray-400 mt-1 ml-1 hidden sm:block">
      ...
      {showNewChat && (
        ...
        >
          <div
            className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:w-96 max-h-[75vh] flex flex-col shadow-xl"
          ...
          >
            ...
            <p className="text-xs text-gray-400 px-5 pt-3 pb-1 font-medium uppercase tracking-wide flex-shrink-0">
              Choose a faculty
            </p>
